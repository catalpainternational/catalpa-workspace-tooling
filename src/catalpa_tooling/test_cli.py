...
from catalpa_tooling.config import ProjectConfig, resolve_frontend_package_manager
from catalpa_tooling.frontend_pkg import package_run_cmd
...
def _run_pytest(extra: list[str]) -> int:
    cfg = _config()
    cmd = ["uv", "run", "--group", cfg.native.test.group, "pytest", *extra]
...
def _run_vitest(extra: list[str]) -> int:
    ...
    package_manager = resolve_frontend_package_manager(
        cwd, configured=cfg.native.frontend.package_manager
    )
    argv = package_run_cmd(package_manager, cfg.native.test.frontend_script, args=extra)
    nvm = Path.home() / ".nvm" / "nvm.sh"
    if nvm.is_file() and (cwd / ".nvmrc").is_file():
        quoted = " ".join(shlex.quote(part) for part in argv)
        cmd = ["bash", "-lc", f"source {nvm} && nvm use && {quoted}"]
    else:
        cmd = argv
